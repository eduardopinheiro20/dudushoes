...
# ADCIONA OS PRODUTOS NO CARRINHOS DE COMPRAS
...
#CRIA VIZUALIZAÇÃO DO CARRINHO DE COMPRAS
...
class CheckoutView(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if session_key and CartItem.objects.filter(cart_key=session_key).exists():
            ...
            cart_items.delete() #remove os elementos do carrinho apos escolher forma de pagamento
        else:
...
class OrderDetailView(LoginRequiredMixin, DetailView): #Ira detalhar um determinado Objeto
...
def pagseguro_notification(request):
    ...
    if notification_code:
        pg = PagSeguro(
            ...
            config={'sandbox': settings.PAGSEGURO_SENDBOX}
        )
